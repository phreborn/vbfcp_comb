...
class splitter {
    public:
        ...
        void makeSnapshots(std::string minimizerType, double tolerance = 0.001, bool singlePoi=true, int fitFlag=0);
        void makeNominalAsimov();
        void makeInjection(
                std::string injectFile,
                std::string injectMass,
                std::string injectStr,
                bool simpleScale = true,
                bool skipMakeDataset = true,
                std::string givenSBDataName="asimovData_1"
                );
