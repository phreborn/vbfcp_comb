...
{
  std::cout<<"************* REGTEST: Entering function asimovUtils::asimovDatasetWithFit *************"<<std::endl;
  std::cout<<"REGTEST: Whether there are multiple POIs: "<<multiplePoi_<<std::endl;
  ...
  if ( poiValue<-99 ) {
    if(multiplePoi_){
      std::cout<<"REGTEST: Modifying multiple POIs"<<std::endl;
      TIterator *iter = poi.createIterator();
      RooRealVar* parg = NULL;
      while((parg=(RooRealVar*)iter->Next()) ){
	parg->setConstant(false);
      }
      SafeDelete(iter);
    }
    else r->setConstant(false);
  } else {
    if(multiplePoi_){
      std::cout<<"***************** REGTEST: Modifying multiple POIs *****************"<<std::endl;
      TIterator *iter = poi.createIterator();
      RooRealVar* parg = NULL;
      while((parg=(RooRealVar*)iter->Next()) ){
	parg->setVal(poiValue);
	parg->setConstant(true);
	std::cout<<"Set variable "<<parg->GetName()<<" to "<<parg->getVal()<<" and set it "<<(parg->isConstant()?"fixed":"free")<<std::endl;
      }
      SafeDelete(iter);
    }
    else{
  ...
  std::cout<<"REGTEST: generating Asimov data"<<std::endl;
...
{
  std::cout<<"REGTEST: Whether there are multiple POIs: "<<multiple<<std::endl;
  multiplePoi_=multiple;
  ROOT::Math::MinimizerOptions::SetDefaultMinimizer(minimizerType.c_str());
  ...
  if ( doZero ) {
    ...
    assert( asimovData );
    std::cout<<"REGTEST: Asimov dataset validated"<<std::endl;
    ...
    std::cout<<"REGTEST: Asimov dataset imported"<<std::endl;
